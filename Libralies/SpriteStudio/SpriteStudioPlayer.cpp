...
class AnimationSprite
{
	...
	public:
		...
		//! @param[in] isFlip : 画像の左右反転を行うかのフラグ
		//!
		//! @return なし
		//------------------------------------------------------
		void Draw(int x, int y, double scale, double angle, bool isFlip)
		{
			m_ssplayer->setPosition((float)x, (float)y);
			m_ssplayer->setScale((float)scale, (float)scale);
			m_ssplayer->setRotation(0.0f, 0.0f, (float)ToDegrees(angle));
			m_ssplayer->setFlip(isFlip, false);
...
// ファイルスコープ変数の定義 ==============================================
std::unordered_map<int, std::string> f_animationPackNameList;    //!< アニメーションパック名のリスト
std::unordered_map<int, ASpritePtr>  f_animationSpriteList;      //!< アニメーションスプライトのリスト
...
//! @retval -1：エラー発生
//----------------------------------------------------------------------
int FinalizeSpriteStudioPlayer()
{
...
int DrawASprite(int x, int y, HASPRITE ASpriteHandle, int TransFlag)
{
	...
	if (it != f_animationSpriteList.end())
	{
		it->second->Draw(x, y, 1.0, 0.0, false);
		return 0;
	}
	else
	{
...
int DrawTurnASprite(int x, int y, HASPRITE ASpriteHandle, int TransFlag)
{
	...
	if (it != f_animationSpriteList.end())
	{
		it->second->Draw(x, y, 1.0, 0.0, true);
		return 0;
	}
	else
	{
...
int DrawExtendASprite(int x, int y, double ExtRate, HASPRITE ASpriteHandle, int TransFlag)
{
	...
	if (it != f_animationSpriteList.end())
	{
		it->second->Draw(x, y, ExtRate, 0.0, false);
		return 0;
	}
	else
	{
...
int DrawRotaASprite(int x, int y, double ExtRate, double Angle, HASPRITE ASpriteHandle, int TransFlag, int TurnFlag)
{
	...
	if (it != f_animationSpriteList.end())
	{
		it->second->Draw(x, y, ExtRate, Angle, TurnFlag != 0);
		return 0;
	}
	else
	{
